  ...
  <div v-else :class="containerClass">
    ...
    <section :class="listSectionClass">
      <div v-for="item in items" :key="item.id ?? item.product_option_id"
        class="grid gap-3 p-3 bg-white border border-[#f1f0ed] rounded-xl items-center"
        :class="itemGridClass"
      >
        ...
        <div class="flex flex-col justify-center min-w-0">
          ...
          <div class="text-xs text-[#67645e] mt-2 block sm:hidden">
             {{ item.qty }} x {{ item.price < item.original_price ? formatTwd(item.price) : formatTwd(item.original_price) }}
          </div>
          <div v-if="layoutMode === 'page'" class="hidden sm:block text-sm text-[#67645e] mt-1">
        ...
        <div class="flex justify-end self-baseline">
...
const listSectionClass = computed(() => {
    ...
    return 'flex-1 overflow-y-auto px-2 space-y-3 py-4 bg-[#fafafa]'
})
...
const summaryContainerClass = computed(() => {
    ...
    return 'border-t border-stone-300 px-4 py-3 bg-white'
})
