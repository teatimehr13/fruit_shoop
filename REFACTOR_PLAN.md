...
- **[決策]** 使用者要求 `/products` 頁改版,直接沿用 Home 首頁「人氣商品」的卡片設計,並把卡片從 `Home/_TopProducts.vue` 抽取成共用元件給 Products 頁一起用。動工前確認兩個範圍問題:①Products 頁原本有原價劃線的折扣顯示(DB 現況有 35 筆商品選項正在折扣中,是 live 功能不是死碼),但 Home 卡片設計原本沒有這個 UI——使用者選擇**在共用卡片補上折扣 UI**,不犧牲 Products 頁既有功能;②抽出共用元件後 **Home 頁自己也一併改用**,不是只抽出來給 Products 單邊用,避免兩邊各自維護一份卡片以後又長歪。
- **[改動]** 新增 `DaisyComponents/Front/ProductCard.vue`,以 Home 卡片(方形圖 `object-contain`、`rounded-[20px]` 邊框卡片、圓形快速加購按鈕、規格滑出面板)為基礎,重新設計成單一商品的自包含元件:`product` 為唯一 prop,內部自己 inject `openCart`、管理選中規格(`selectedOptionId`)、面板開關(`isPanelOpen`)、加入購物車邏輯(`axios.post(cart.store)` + `openCart()` + `router.reload`),父層只要 `v-for` 帶入 `product` 即可,不用再各自維護一份選項/面板狀態。因為面板開關跟遮罩狀態現在各卡片自己管,不再共用同一個 `selectToCartId`,原本「同時間只能開一張卡片面板」的效果改成靠**遮罩自然攔截點擊**達成(開啟中的卡片有自己的全螢幕遮罩蓋住其他卡片,手機版點別的商品按鈕會先點到遮罩把目前這張關掉,而不是直接切換開另一張)——效果上等價,不是真的多卡片可以同時開著。
- **[改動]** 資料形狀統一:兩邊 controller 回傳的欄位本來不一樣(Home 的 `home()` 是手動 `map()` 出扁平 `image` 欄位;Products 的 `index()` 是完整 Eloquent model,圖片在巢狀的 `primary_image.img_url`)。沒有強改後端統一成同一形狀,改成讓 `ProductCard.vue` 自己相容兩種(`product.image || product.primary_image?.img_url`),因為 Products 頁的 `index()` 欄位其實被其他地方也依賴(如 `has_discount`/`cheapest_price` 等 `$appends`),牽動範圍較大,不值得為了共用卡片重構。折扣邏輯則統一改成**看目前選中規格的 `original_price` vs `price`** 現算(`selectedOption.original_price > selectedOption.price`),不是採用 Products 原本「固定顯示最低價那個 option 的折扣、不管使用者選了哪個規格」的寫法——這樣切換規格時折扣顯示會跟著正確更新,比原本的行為更精確,也剛好讓 Home/Products 兩邊邏輯一致。`ProductController@home()` 補上 `original_price` 到 `productOptions` 的 select 欄位(原本只選 `id,product_id,option_text,price`),不然 Home 頁的商品永遠算不出折扣。
- **[改動]** `Products/Index.vue`:拿掉舊卡片 markup 跟對應的一整組 script(`selectedOptions`/`selectToCartId`/`quickAddToCart`/`confirmAddToCart`/`submitToCart`/`getSelectedOption`/`handleOptionChange`/`formatTwd`,共用元件內部自己處理掉了),grid 直接換成 `<ProductCard v-for="value in props.products" :key="value.id" :product="value" />`;grid 本身欄數(`grid-cols-2 gap-3 md:gap-5 xl:grid-cols-3`,含側邊分類欄)維持 Products 頁原本的版面配置不動,使用者要的是「卡片設計」不是連版面欄數也套用 Home 的 2/4 欄。順手清掉檔案開頭殘留的 3 個除錯用 `console.log`(印 `categories_tab`/`products`/`activeCategory`),跟卡片重構無關,是既有殘留。
- **[改動]** `Home/_TopProducts.vue` 同步改用 `ProductCard`,section 只留標題/CTA 按鈕跟 grid wrapper,原本卡片相關的 script 邏輯(跟上面 Products 頁拿掉的是同一套)全部移除。兩個頁面原本各自獨立定義一份的 `.bg-select-arrow`/`.options-close` 全域 CSS(Phase 2 決策紀錄裡提過「不同頁面各自 code-split 不會共用,需要重複定義」),現在因為都改吃同一個 `ProductCard.vue`,這兩個 class 只需要在元件裡定義一次,兩邊頁面的重複 `<style>` 區塊都拿掉了。
- **[驗證]** 用 Playwright 分別測 `/products`(桌機 1400×1000、手機 390×844)跟 `/`(桌機),確認:折扣商品正確顯示劃線價(`/products` 量到 18 個 `.line-through` 元素,Home 頁「紅石榴汁」等原本沒有折扣顯示的商品現在也正確顯示);單一規格商品(芹菜汁)點擊快速加購直接送出、購物車抽屜正確打開;多規格商品(`/products` 頁的「羽衣甘藍」、Home 頁的「韓國麝香葡萄」)點擊會滑出選項面板,選項/縮圖/加入按鈕金額都正確;`npm run build` 編譯成功,全程 console 無錯誤。
- **[決策]** `/products` 頁整體版面重新設計。動工前先用 Playwright 實際訪問參考站 Econis 的 `/shop/` 頁(直接 WebFetch 拿到 403,改用瀏覽器實際渲染截圖,跟 Phase 2 拉 Elementor CSS 是同一招),另外 WebSearch 查了 2026 年電商商品列表頁的通用原則(桌機 3-4 欄/手機 2 欄、手機篩選建議用 filter drawer)。發現 Econis 的完整 WooCommerce shop 頁(側邊欄手風琴+價格滑桿+顏色/尺寸篩選+評分星星+收藏愛心)牽涉到本站沒有的資料(只有 3 個分類、沒有商品評分/收藏功能、沒有顏色尺寸這種商品屬性),照搬會做出一堆假資料撐場面的空殼 UI,因此提供 3 個扣掉這些不適用元素後的方向(A 維持側邊欄骨架做視覺升級/B 拿掉側邊欄改頂部分類+滿版網格/C 桌機側邊欄+手機底部篩選抽屜)讓使用者選。使用者選 **A**,並明確縮小範圍:卡片(已經是上一輪抽出來的 `ProductCard.vue`)不用再動,只要改**最上方 banner**、**側邊欄**,並且**新增獨立的麵包屑元件**。
- **[改動]** 新增 `DaisyComponents/Front/Breadcrumb.vue`:吃 `items: [{label, href?}]` prop,最後一項(或沒有 `href` 的項目)視為目前頁面,不當連結、用 primary 色標示,其餘項目是灰色連結、hover 變 primary,中間用 chevron SVG 分隔。放在 Nav 正下方的淺灰底色窄條(呼應 Econis 麵包屑條的位置),扛起 `mt-[var(--spacing-header-space)]`(原本是掛在 PageHero 上,因為麵包屑現在變成 `<main>` 底下第一個元素,要接手把內容推到固定 Nav 下方的責任)。`Products/Index.vue` 用 `computed` 依 `categoryName` 動態組麵包屑(`首頁 > 所有商品` 或 `首頁 > 所有商品 > {分類名}`)。
- **[改動]** `Products/_PageHero.vue` banner 重新設計:高度從 `aspect-[16/9] md:h-[500px]`(桌機 500px 對一個純標題橫幅來說偏高,比較像首頁 hero 不像頁面橫幅)縮到 `h-[220px] md:h-[320px]`;文字從置中改成手機置中/桌機靠左(`text-center md:text-left`,呼應 CTA 區已經確立的手機置中桌機靠左寫法);暗化遮罩從單純的 `bg-gradient-to-r from-black/50 to-transparent` 加深漸層層次(`from-black/65 via-black/35 to-transparent`),讓左側文字對比度更夠;標題字距補上 `tracking-[0.02em]`(沿用 CTA 標題查證過的 CJK 排版做法),字重維持 `font-medium`。順手清掉檔案裡的死程式碼:`heroRef`/`setHeroRef` 是從 Home 的 hero 複製貼上但忘記接好的殘留(`setHeroRef` 函式整個被註解掉,`:ref="setHeroRef"` 卻還留著,實際上從來沒把這個 banner 元素交給 Nav 的穿透效果機制),因為這個頁面本來就不需要「捲動穿過 hero 才變色」的 Nav 效果(現況 Nav 在這頁本來就是不透明白底,截圖確認拿掉後視覺沒有任何變化,純粹是清掉一段從未真正生效過的死碼)。
- **[改動]** 側邊欄「產品分類」重新設計:容器從 `bg-base-200 rounded-[12px]` 換成跟 `ProductCard`/CTA 卡片同一套語言的 `bg-base-100 border border-base-300 rounded-[20px]`;標題底線從 `before:` 偽元素手刻改成單純 `border-b`;分類項目的選中狀態從實心色塊 `bg-primary text-base-100` 改成跟 `Nav.vue` 手機選單同一套已確立的軟色調互動樣式(`bg-primary/10 text-primary` 選中、`hover:bg-primary/10 hover:text-primary` 未選中 hover),視覺上跟整站其他互動元素(手機選單連結、CTA 按鈕 hover)語言一致,不是突兀的實心色塊。
- **[驗證]** 用 Playwright 測桌機(1400×1000)、手機(390×844):首頁 banner 縮短、文字桌機靠左手機置中都正確;點側邊欄「蔬菜」分類,截圖確認麵包屑正確從「首頁 > 所有商品」變成「首頁 > 所有商品 > 蔬菜」,側邊欄選中項目變成綠色軟底色;手機版麵包屑條、banner、pill 分類列都正常顯示;全程 console 無錯誤,`npm run build` 編譯成功。
- **[決策]** 使用者對上一輪 `/products` 改版「差強人意」,要求再找模版參考。因為 WebFetch 對大部分正式電商站(Sprouts/Sakara/Thrive Market)都是 404/loading/被擋,改找靜態 HTML 範本(GitHub Pages 上的免費 Shopify/HTML template demo,例如 `themewagon.github.io/fruitables`、`themewagon.github.io/foody2`)搭配少數真的能訪問的正式站(BoxedGreens、Econis 既有參考),挑 5 個風格光譜不同的做成一頁截圖對照 Artifact 給使用者看。使用者選定:麵包屑跟 banner 排版**完全比照模版3(Foody)**——banner 從照片橫幅改成淺色底色塊,標題「所有產品」+麵包屑緊接在標題正下方(不是獨立色塊);其餘(側邊欄以外的所有互動元素)**比照模版1(BoxedGreens)**——拿掉側邊欄,分類選擇改成「篩選」按鈕。卡片維持不動。動工前用 AskUserQuestion 確認「篩選」按鈕點了之後要長怎樣(BoxedGreens 截圖沒拍到展開狀態),使用者額外截圖附上實際點開的畫面(左側抽屜,標題「Filter」+ X 關閉+分類清單),因此確認要做**左側抽屜**,而且**手機版也要一併改成篩選按鈕**(不維持原本的 pill 分類列),banner 右側**要放裝飾照片**。
- **[改動]** `Products/_PageHero.vue` 整個重寫成 Foody 排版:banner 從全螢幕照片+深色遮罩,改成 `bg-base-200` 淺色色塊,左側是標題「所有產品」(`text-3xl md:text-4xl font-medium`)+ 麵包屑緊接在下方(用 `Breadcrumb` 元件的新 `bare` 模式,不帶外層底色/邊框,純文字疊在色塊上),右側用既有的 `veggie-box.webp`(促銷 banner 用過的同一張)當裝飾照片,`absolute right-0` 貼右邊界、`rounded-t-[28px]` 只留左上圓角,手機版隱藏(`hidden md:block`,手機寬度放不下側邊裝飾圖)。
- **[改動]** `DaisyComponents/Front/Breadcrumb.vue` 補上 `bare: Boolean` prop:`bare=true` 時不套外層 `bg-base-200/60 border-b` 色塊條跟 `mt-[var(--spacing-header-space)]` 頂部間距,只留純文字清單,讓 `_PageHero.vue` 疊在自己的色塊裡;`bare=false`(預設)維持上一輪「獨立色塊條」的樣式,元件還是同一個,不是複製一份改樣式,之後若有其他頁面要用「獨立色塊條」版本可以直接沿用。原本掛在 `Breadcrumb.vue` 上的 `mt-[var(--spacing-header-space)]`(撐開固定 Nav 高度)因為 `bare` 模式下拿掉了,改掛回 `_PageHero.vue` 的最外層 `<section>`(因為 banner 現在又變回 `<main>` 底下第一個元素)。
- **[改動]** 拿掉側邊欄「產品分類」跟手機版 `_PillsSwiper.vue`(整個檔案刪除,確認過站內沒有其他頁面引用),改成單一個「篩選」按鈕(邊框膠囊+篩選圖示),跟「27 件商品」件數、排序下拉三個元素平鋪同一列(比照 BoxedGreens 的 Filter/count/sort 橫排),網格拿掉側邊欄讓出的空間後改滿版寬度,欄數從 `xl:grid-cols-3`(含側邊欄)拉到 `sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5`。
- **[改動]** 新增 `Products/_FilterDrawer.vue`,機制直接照抄 `Nav.vue` 手機選單那套已經驗證過的 offcanvas 做法(`Teleport to body` + 背景遮罩 `Transition` + 抽屜本體 `-translate-x-full` 滑入 `Transition`,理由一樣是 header 的 transform 會讓子孫 `fixed` 定位跑掉),不是重新設計一套新機制。抽屜內容是單層分類清單(全部/蔬菜/水果/果汁),沿用上一輪側邊欄已經確立的軟色調選中樣式(`bg-primary/10 text-primary`)。因為我們的分類本來就是扁平的(不像 BoxedGreens 截圖那樣有巢狀子分類的手風琴結構),沒有照抄那層複雜度,只做符合本站實際資料形狀的版本。桌機、手機共用同一顆「篩選」按鈕跟同一個抽屜元件,不再分桌機側邊欄／手機 pill 兩套。
- **[驗證]** 用 Playwright 測桌機(1400×1000)、手機(390×844):banner 標題+麵包屑疊在淺色色塊裡、桌機右側裝飾圖正確顯示且貼齊右邊界;點「篩選」按鈕,桌機/手機都正確從左側滑出抽屜(截圖比對跟使用者附的參考截圖版面一致);點分類「蔬菜」,麵包屑正確變成「首頁 > 所有商品 > 蔬菜」、商品從 27 件篩到 8 件、抽屜自動關閉;網格在桌機正確跑滿 5 欄;全程 console 無錯誤,`npm run build` 編譯成功。
- **[決策]** 使用者記得另一種篩選做法:按鈕點下去面板直接掛在按鈕正下方展開(不是側邊抽屜),清單樣式類似 radio(文字+圓點)。上網查了實際範例(Flowbite 元件庫的 React Dropdown Filters 頁面,`shopalytics.app/application-ui/dropdown-filters`,裡面每個「Filter by ...」按鈕點下去都會在正下方彈出小面板,截圖了兩個變體給使用者看:一個是 checkbox 卡片式清單、一個是純文字 radio 清單),使用者確認要的是後者(純文字圓點+文字)。動工前確認兩個細節:選完分類要**自動關閉**面板(不用像多選篩選那樣按「套用」),分類名稱後面要**顯示件數**。
- **[改動]** 拿掉上一輪做的 `_FilterDrawer.vue`(左側抽屜),改成新的 `Products/_FilterDropdown.vue`:按鈕本身管理自己的開關狀態(`isOpen`),面板用 `absolute left-0 top-full` 掛在按鈕正下方,`Transition` 做淡入+輕微下滑;清單項目是圓點(`border rounded-full`,選中時 `bg-primary border-primary`,模擬 radio 外觀但沒用真的 `<input type="radio">`,跟站內其他分類清單一路都是純點擊處理、不是表單提交是一致的)+ 名稱 + 件數;點`document`上任何面板以外的地方(`onMounted` 掛 `click` listener,比對 `event.target` 是否在 `rootRef` 內)會自動關閉,點分類選項後除了觸發 `select-category` 也會自己把 `isOpen` 設回 `false`。因為抽屜元件整個被取代、桌機手機都改用同一顆按鈕,不再需要 `_FilterDrawer.vue`,檔案直接刪除。
- **[改動]** 分類件數是新資料,原本 `categories_tab` 只有 `id`/`name`,沒有商品數。`Category` model 補上 `products()`(`hasManyThrough(Product::class, Subcategory::class, 'category_id', 'subcategory_id')`,因為 `Category → Subcategory → Product` 是兩層關聯,Laravel 沒有內建語法能直接 `withCount` 穿過中間表,要自己定義這個 `hasManyThrough`);`ProductController@index()` 的 `$categories_tab` 查詢補上 `->withCount(['products' => fn($q) => $q->where('products.is_enabled', 1)])`。中途踩到一個小坑:一開始寫 `$q->where('is_enabled', 1)` 直接噴 `SQLSTATE[23000]... Column 'is_enabled' in where clause is ambiguous`,因為 `withCount` 產生的 subquery 同時 join 了 `categories`(自己也有 `is_enabled` 欄位)跟 `products`,兩邊都有同名欄位,改成明確寫 `products.is_enabled` 才解決。另外「全部」這個選項的件數不能用畫面上當下 `products.length`(因為那是被分類篩過的清單,選了「蔬菜」之後就只剩 8),另外查一個不受篩選影響的 `$totalProductsCount = Product::where('is_enabled', 1)->count()` 當作固定總數傳給前端。用 tinker 驗證過三個分類的件數(蔬菜 8、水果 11、果汁 8)加總剛好等於總數 27,沒有算重複或漏算。
- **[驗證]** 用 Playwright 測桌機(1400×1000)、手機(390×844):點「篩選」面板正確掛在按鈕正下方展開(不是側邊抽屜),圓點+文字+件數樣式跟使用者看過的參考截圖一致;點「蔬菜」後面板自動關閉、商品篩到 8 件、麵包屑更新;重新展開面板後點空白區域(麵包屑列附近,刻意避開任何連結)確認面板正確關閉且沒有誤觸發導航;手機版同一套互動正常;全程 console 無錯誤,`npm run build` 編譯成功。
- **[改動]** 排序 `<select>` 樣式一直對不齊「篩選」按鈕(高度、padding、字重、hover 都得手動一一調整才能勉強貼近,而且原生 select 被 Tailwind Preflight reset 成 `appearance:none` 導致下拉箭頭消失,還得補一個 `.bg-select-arrow` 自訂箭頭 CSS class),使用者建議乾脆做成跟 `_FilterDropdown.vue` 同一套「按鈕+絕對定位面板」元件,保證樣式一致。新增 `Products/_SortDropdown.vue`,結構直接照抄 `_FilterDropdown.vue`(按鈕自己管 `isOpen`、面板 `absolute top-full` 掛在按鈕下方、`document` click-outside 自動關閉),差異只在面板改用 `right-0` 對齊(排序按鈕在 toolbar 最右側,面板要往左展開才不會超出螢幕),按鈕文字改顯示目前選中的排序名稱(`{{ modelValue?.label }}`)取代原本固定的「篩選」文字。父層用 `@select="onSelectSort"` 接住選到的 option 物件,邏輯跟 `onSelectCategory` 同一個模式:設定 `selectedSort.value` 後呼叫同一個 `sortData()`。原生 `<select>` 跟補的 `.bg-select-arrow` CSS 都拿掉了。之後使用者又要求把兩個下拉選單裡原本模擬 radio 外觀的圓點(`<span class="rounded-full border">`)拿掉,只留文字+選中變色,`_FilterDropdown.vue`、`_SortDropdown.vue` 都同步拿掉。
- **[改動]** `ProductCard.vue` 卡片圖片拿掉 `max-w-[75%] max-h-[75%]`,圖片改滿版填滿卡片的正方形區域(只留 `object-contain` 讓長寬比不失真)。因為是共用元件,Home 首頁人氣商品跟 Products 頁一起套用。
- **[決策]** `/products` banner 圖片使用者提供一張新素材(白底、食材集中在畫面右側、左側大片留白),複製進 `public/images/hero/4c215f43-525a-48d8-9878-95e0599a6aa2.webp`(沿用 `hero/` 資料夾既有的雜湊檔名慣例)。動工前用 AskUserQuestion 確認用法:因為這張圖左側留白剛好跟 banner 左側放標題文字的版面對齊,選擇**當成整個 banner 背景圖**(而不是像上一版那樣只當右側窄長裝飾圖,那樣做會裁掉圖片大部分內容)。`_PageHero.vue` 整個重寫:拿掉 `bg-base-200` 淺色色塊+右側窄長裝飾圖的結構,改成全版 `object-cover` 背景圖,標題「所有產品」+麵包屑疊在圖片左側的留白處,因為背景本身夠亮、文字夠深(`text-heading`),不需要暗化遮罩就有足夠對比度。banner 高度從 `h-[220px] md:h-[320px]` 微調到 `h-[260px] md:h-[380px]`,用 Playwright 桌機/手機截圖確認文字在兩種尺寸下都清楚落在圖片留白區、沒有被食材擋住。
- **[改動]** 使用者反饋圖片滿版的 banner 主體(食材)還是佔畫面太大,問是否要重新出圖。解釋這其實是 CSS 問題(`object-cover` 在很寬很矮的 banner 容器上會把圖片放大裁切,容器跟圖片長寬比差越多、裁切越兇),不需要重新出圖,先試了「圖片四周留白、框在 `bg-base-200` 裡不貼滿邊緣」的做法(`inset-6 md:inset-10` + `rounded-2xl`)。使用者後來還是決定自己重新出一張圖,跟他要「下什麼指令給 AI」,提供了英文 prompt(重點:主體只佔約 30%、集中在右側三分之一、其餘留白、寬版 21:9 構圖)。使用者用這個方向重新產了一張圖(`1916×821`,實測比例約 2.33:1,主體確實只佔右側一小塊、左側大量留白),換成新圖後拿掉先前的留白框做法、改回單純 `object-cover` 滿版鋪滿(因為新圖本身的留白已經足夠,不需要再靠 CSS 裁切補),舊圖檔案跟著刪除。
- **[決策]** 使用者接著問「這種 banner 是不是過氣設計」,先給主觀判斷(banner 對商品列表頁功能性低、拉長捲動距離),使用者要求上網查證。查到三個支持拿掉/簡化的依據:①**Baymard Institute** 明確指出分類頁用大 banner 把商品/篩選擠到第一屏下面是「最常見也最傷的錯誤」,手機版尤其嚴重(banner 可能佔滿整個手機第一屏);②**Nielsen Norman Group** 的眼動追蹤研究顯示使用者 80% 的瀏覽時間花在第一屏以上,核心功能不該需要捲動才看到;③業界慣例把分類頁分成「editorial-heavy」(banner+品牌故事並行,常見於時尚/生活風格品牌,例如 Zara)跟「pure navigation」(直接列分類+商品,幾乎沒有編輯內容,適合使用者是來快速買東西的實用型零售商)兩種,生鮮雜貨屬於後者。使用者確認後直接拿掉。
- **[改動]** `/products` banner 整個拿掉,`_PageHero.vue` 檔案跟裡面用的 banner 圖片(`91d49e47-3884-4859-946d-18447b1a9e69.webp`)一併刪除,確認過站內沒有其他地方引用這兩者。改成頁面最上方一個極簡的標題區塊:麵包屑(`Breadcrumb` `bare` 模式)在上、標題「所有產品」在下,直接接著 toolbar(篩選/件數/排序)跟商品網格,不再有照片橫幅、不再需要額外一段 `mt-[var(--spacing-header-space)]` 撐開固定 Nav 高度的獨立 section(改掛在這個標題區塊的最外層 `<div>` 上)。用 Playwright 驗證桌機/手機:商品網格現在幾乎是第一屏就看得到(手機版尤其明顯,原本要捲過一整張照片才看得到商品),點分類切換、麵包屑更新都正常,`npm run build` 編譯成功,全程 console 無錯誤。
- **[改動]** 使用者接連要求幾個標題區塊的細部調整:①麵包屑改放到標題下面(原本在上面)、分隔符號從 chevron SVG 換成純文字 `/`(`Breadcrumb.vue` 補上這個改動,同時補上 `bare` 模式讓母層可以自己控制外層有沒有底色/邊框/置頂間距);②工具列的 `border-b`(分隔線)從 toolbar 移到標題+麵包屑區塊下方,讓標題區有明確收尾,不是整段空白硬撐距離(查了 IBM Carbon、EU 官方 design system 的 Page Header 規範,兩者都建議標題區塊該有收尾分隔線、且麵包屑跟標題間距不宜過鬆);③「27 件商品」的數字字級加大到 `text-[20px]`。踩到一個小坑:分隔線一開始跟 `px-4` padding 掛在同一個 div 上,border 畫在 padding 外側,導致線比文字內容寬(手機版線甚至直接貼到螢幕邊緣、跟上面內縮的標題文字不對齊),使用者反饋「這個線也要有 padding」後,改成外層 div 只負責 `max-w-layout-wide` + `px-4` 定寬,border 移到內層沒有自己 padding 的 div 上,讓線的寬度自然等於已經內縮過的內容寬度,不會比文字更寬。
- **[改動]** 使用者回報 Nav 購物車圖示的角標數字跟購物車 offcanvas 抽屜標題「購物車 (N)」的數字對不上。查出原因:兩邊各自算了不同的東西——`Nav.vue`(透過 `useSharedCart.js` 的 `ItemsCount`)顯示的是 `cartItems.total_qty`(後端 `CartService.php` 算好的**所有品項數量加總**),`CartDrawer.vue` 卻是自己另外算 `cartItems.value?.items?.length`(**不重複的品項筆數**)。兩邊資料源頭其實是同一個 Inertia shared prop(`cartItems`,經 `HandleInertiaRequests` middleware 共用),不是資料沒同步的問題,純粹是兩邊拿去顯示的欄位不一樣——只要購物車裡任何一筆品項數量大於 1,兩個數字就會兜不起來。修法是讓 `CartDrawer.vue` 也改用 `useSharedCart()` 已經曝露出來的 `ItemsCount`(跟 Nav 用同一個),不要自己另外算 `items.length`。用 Playwright 驗證:同一件商品快速加購兩次讓某筆品項數量變成 2,確認 Nav 角標跟抽屜標題都正確顯示「(2)」,不再顯示不同的數字,全程 console 無錯誤。
- **[改動]** 使用者反饋 Footer 最大寬度要對齊 Header。查出原因:`Footer.vue` 外層 `<footer>` 用 `p-10`(四邊都 40px)當水平間距,跟站內其他區塊(Nav、Products 頁各 section)統一用的 `max-w-layout-wide mx-auto px-4`(16px)不一致,導致兩邊容器雖然都是 `max-w-layout-wide` 同寬,但內容實際起始位置差了 24px(40px vs 16px)。改成外層只留 `py-10`(垂直間距不變),水平間距移到內層 `.footer` div 上、改用一致的 `px-4`。用 Playwright 量測 header 跟 footer 第一個文字元素的 x 座標,確認桌機(1400px)都對齊在 x=16,手機版也驗證過內容邊界對齊上方商品網格,全程 console 無錯誤。
- **[決策]** 使用者要求把 `Products/Show.vue`(商品詳情頁)版面對齊 `boxedgreens.com/products/apples-organic-fuji` 這個參考頁,先討論範圍不執行。用 Playwright 截圖比對雙方版面差異,列出參考頁有、我們沒有的東西:價格緊接標題下方(我們原本在描述/規格之後)、**數量選擇器**(我們完全沒有,只能一次加 1 件)、規格用方框不是圓角膠囊、描述前有「About the Product」小標題、獨立的「You may also like」相關商品區塊、訂閱制/評論/社群分享(我們沒有對應資料,建議跳過)。使用者確認整個範圍都可以做,唯獨圖片區另外討論。
- **[決策]** 圖片區(多圖情境參考頁沒展示到)另外查了 PDP 圖片區三種常見做法:①縮圖列(我們原本的做法,但用 JS 手動量高度同步桌機直排/手機橫排兩套邏輯,偏複雜)、②只留主圖+圓點指示器+箭頭,不要縮圖列(Apple 的做法,截圖給使用者看)、③手機純滑動+桌機縮圖列的混合做法。使用者選 **②**,理由是最符合「簡潔感」,而且站內大多數商品圖片數量本來就不多(1-2 張居多),縮圖列的功能性有限。
- **[改動]** `ProductController@show()` 補三項資料給前端:①`subcategory.category` 預載入,用來組麵包屑(首頁 > 所有商品 > {分類} > {商品名});②新查詢 `$relatedProducts`(相關商品,限 5 筆,`is_enabled=1`、排除自己);③把 `categoryName` 一起傳出去。踩到一個資料面的坑:相關商品一開始用 `subcategory_id` 篩,結果「羽衣甘藍」這個商品所在的子分類「葉菜」底下只有它自己一筆,區塊整個是空的——用 tinker 查資料才發現分類底下其實有多個子分類(蔬菜下面有根莖瓜果/菇蕈/葉菜/辛香料四個子分類),範圍抓子分類太窄,改成抓「同一個分類底下所有子分類的商品」(`whereHas('subcategory', fn($q) => $q->where('category_id', $categoryId))`,跟 `index()` 既有的分類篩選寫法同一招),相關商品區塊才會有內容。
- **[改動]** `Products/Show.vue` 整個重寫:①拿掉整個 Swiper 縮圖輪播機制(`swiper--thumbs`、`updateThumbsHeight` 那組 JS 手動量高度同步的邏輯全部刪除),改成單張主圖+圓點指示器+左右箭頭按鈕(選中的圓點用 `w-6 bg-primary` 加寬,其餘 `w-2` 純圓點),圖片只有 1 張時圓點/箭頭整組不顯示;②價格搬到標題正下方,補上跟 `ProductCard.vue` 同一套折扣判斷邏輯(`selectedOption.original_price > selectedOption.price` 才顯示劃線價);③規格選項從圓角膠囊(`rounded-full`)改成 `rounded-[4px]` 方框,跟站內其他按鈕(篩選、排序、標籤頁的圓角語言)统一,選中狀態用 `border-primary bg-primary/5` 淡底色而不是實心色塊;④新增數量選擇器,直接沿用購物車既有在用的 `QuantityStepper.vue`(站內另外還有一個沒人用的 `QuantityStepper_Product.vue`,沒去動它,不在這次任務範圍內);⑤描述前面補「商品介紹」小標題;⑥麵包屑用 `Breadcrumb` 元件的 `bare` 模式;⑦最下面新增「你可能也喜歡」相關商品區塊,直接重用 `ProductCard.vue`(有相關商品才顯示區塊,`v-if="relatedProducts?.length"`)。這次重寫也移除了檔案開頭殘留的 `console.log(props.product)` 除錯輸出跟一大串沒在用的假圖 `images` 陣列(舊版寫死的 picsum.photos 隨機圖,新版直接吃 `product.product_images`)。因為整個頁面不再用到 Swiper,這個 route 的 JS chunk 從原本要打包 Swiper 全部模組縮到 5.53 kB。
- **[驗證]** 用 Playwright 測單圖商品(芹菜汁,含折扣)、多圖多規格商品(羽衣甘藍):麵包屑正確顯示分類路徑;折扣商品正確顯示劃線價;點左右箭頭正確切換圖片、圓點跟著反白;切換規格價格/折扣即時更新;數量 +/- 正常、`disabled` 邊界正確;加入購物車後金額/數量正確反映在購物車抽屜;相關商品區塊顯示同分類其他商品且可正常點擊;手機版整體排版驗證正常;全程 console 無錯誤,`npm run build` 編譯成功。
